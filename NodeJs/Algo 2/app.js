...
function solve(input = "AABBDDBBB", consecutivity = 3) {
  ...
  individual.forEach((ind, i) => {
    ...
    if (prevInd == ind) {
      if (newConsecutive) {
        ...
        let consecs = uniquei.consecutives.filter(
          (x) => x.start == curConsecutiveIndex
        );
        if (consecs.length) consecs[0].count++;
      }
  ...
  let multiples = uniques.filter((x) => x.consecutives.length > 0);
  ...
  multiples.forEach((mul, i) => {
    let lastConsecutive = mul.consecutives[0];
    mul.consecutives.forEach((con, i1) => {
      if (lastConsecutive) {
        ...
        for (let i2 = 0; i2 < lastConsecutive.count; i2++) {
          ...
          console.log("fds", diff, curLast, lastEnd, i2);
  ...
  splitables.forEach((able, i) => {
    ...
    individuals.forEach((ind) => {
      if (uniques.filter((x) => x == ind).length == 0) uniques.push(ind);
    });
    console.log("uniques", uniques);
    let compressed = uniques.map((x) => {
      return { char: x, count: individuals.filter((y) => y == x).length };
    });
  ...
  if (leader) {
    result = "";
    leader.forEach((x) => {
      result += x.count == 1 ? x.char : x.count + x.char;
    });
  }
